...
**12비트 ADC의 전체 범위, 즉 드라이버 기본값입니다.** 이 패널의 실측값이 아닙니다. 실제 유효 폭은 `782~3286`, 즉 **61%** 뿐이었습니다. `0~4095`로 480픽셀에 매핑하면,

| 누른 곳 | 계산된 화면 x |
|---|---|
| 왼쪽 끝 | `782/4095 × 480` = **92px** |
| 오른쪽 끝 | `3286/4095 × 480` = **385px** |

**화면 좌우 각 19%, 합쳐서 39%가 죽습니다.** 캐릭터가 화면 끝에 절대 닿지 않는데 에러는 없습니다. 그래서 두 점을 눌러 받는 편이 확실합니다.
